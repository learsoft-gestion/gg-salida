...
func ProcesadosControl(db *sql.DB, id_procesado string, id_modelo int, fecha1 string, fecha2 string, version int, cant_registros int, nombre_control string) error {

	fecha_completa := time.Now()
	fecha_actual := fecha_completa.Format("2006-01-02 15:04:05")

	if id_procesado != "" {
		_, err := db.Exec("update extractor.ext_procesados ep set cant_registros_control = $1, archivo_control = $2, fecha_ejecucion_control = $3 where ep.id_procesado = $4", cant_registros, nombre_control, fecha_actual, id_procesado)
		if err != nil {
			return err
		}
	} else {
		_, err := db.Exec("insert into extractor.ext_procesados (id_modelo, fecha_desde, fecha_hasta, version, cant_registros_salida, archivo_salida, fecha_ejecucion_salida) values ($1,$2,$3,$4,$5,$6,$7)", id_modelo, fecha1, fecha2, version, cant_registros, nombre_control, fecha_actual)
		if err != nil {
			return err
		}
	}
