	...
	return func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "PATCH" {
			...
			for rows.Next() {
				...
				if err = rows.Scan(&modelo.Id_modelo, &modelo.Id_empresa, &modelo.Id_concepto, &modelo.Id_convenio, &modelo.Id_tipo, &modelo.Empresa, &modelo.EmpReducido, &modelo.Concepto, &modelo.Convenio, &modelo.Tipo, &modelo.Nombre, &modelo.Filtro_convenio, &filtroPersonas, &filtroRecibos, &modelo.Formato_salida, &ult_ejecucion, &modelo.Query, &modelo.Archivo_modelo, &modelo.Vigente, &filtroHaving, &modelo.Archivo_control, &modelo.Archivo_nomina); err != nil {
					http.Error(w, err.Error(), http.StatusInternalServerError)
					return
				}
